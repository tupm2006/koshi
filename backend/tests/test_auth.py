...
def test_google_oauth_and_user_management_flow(client: TestClient):
    ...
    payload = base64.urlsafe_b64encode(json.dumps({
        ...
        "email": "google.dev@felixsu.qzz.io",
    ...
    assert auth_data["user"]["email"] == "google.dev@felixsu.qzz.io"
    ...
    assert any(u["email"] == "google.dev@felixsu.qzz.io" for u in user_list)

    # 3. Test PM User role update
    # Login as default seed PM
    pm_login = client.post("/api/auth/login", json={"email": "pm@felixsu.qzz.io", "password": "koshi123"})
