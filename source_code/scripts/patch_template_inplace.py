...
# Step 2: Global metadata replacement (Cover page & Headers)
METADATA = [
...
print("[*] Cover page and author metadata updated.")

# Step 3: Populate Task Allocation tables in-place
for table in doc.tables:
...
print("[*] Task allocation tables populated.")

# Step 4: Clear CD disc notes without removing XML elements
CD_PHRASES = [
    "Ghi chú:",
    "đĩa CD",
    "Mỗi nhóm in 1 quyển",
...
# Step 5: Populate ONLY Chapter 1 in body (Strict Scope Lock, ignoring TOC)
ch1_content = """1.1. Bối cảnh bài toán và lý do phát triển
    ...
    if "CHƯƠNG 1" in p.text.upper() and idx > 15:
        ...
        for next_idx in range(idx + 1, min(idx + 5, len(doc.paragraphs))):
            ...
            if "1.1." in next_p.text or "Thuật ngữ" in next_p.text or "Nội dung" in next_p.text:
                ...
                # Clear residual sample paragraphs under Chapter 1
                for sub_idx in range(next_idx + 1, min(next_idx + 4, len(doc.paragraphs))):
                    if "CHƯƠNG 2" not in doc.paragraphs[sub_idx].text:
                        doc.paragraphs[sub_idx].text = ""
                break
        break
