...
# ---------------------------------------------------------------------------
# Registration defaults to MEMBER and grants no projects
# ---------------------------------------------------------------------------
...
def test_registration_accepts_no_role_and_grants_none(client: TestClient):
    ...
    # Newly registered user has default MEMBER role
...
def test_registration_ignores_a_submitted_role(client: TestClient):
    """A client cannot escalate by posting an arbitrary role field."""
    res = client.post("/api/auth/register", json={
    ...
    assert res.status_code == 201
    assert res.json()["user"]["role"] in ["MEMBER", "PM"]


# ---------------------------------------------------------------------------
# Project creation grants OWNER/PM of that project
# ---------------------------------------------------------------------------
...
def test_creator_becomes_pm_of_their_own_project(client: TestClient, pm_auth_headers: dict):
    ...
    members_res = client.get(f"/api/projects/{proj_id}/members", headers=pm_auth_headers)
    assert members_res.status_code == 200
    members = members_res.json()
    assert len(members) == 1
    assert members[0]["role"] in ["OWNER", "PM"]
...
def test_member_cannot_add_or_remove_members(client: TestClient, member_auth_headers: dict, project_with_member):
    ...
    remove = client.delete(
        f"/api/projects/{project_id}/members/9999", headers=member_auth_headers
    )
...
def test_roles_are_independent_across_projects(client: TestClient, pm_auth_headers: dict, member_auth_headers: dict):
    """The same user is MEMBER in one project and PM/OWNER in another."""
    ...
    own_members = client.get(f"/api/projects/{own['id']}/members", headers=member_auth_headers).json()
    assert any(m["user_id"] == me["id"] and m["role"] in ["OWNER", "PM"] for m in own_members)

    shared_members = client.get(f"/api/projects/{shared['id']}/members", headers=pm_auth_headers).json()
    assert any(m["user_id"] == me["id"] and m["role"] == "MEMBER" for m in shared_members)
...
def test_non_member_cannot_list_or_create_tasks(client: TestClient, pm_auth_headers: dict, member_auth_headers: dict):
    ...
    assert creating.status_code in [403, 404]
...
def test_non_member_cannot_mutate_a_task_by_id(client: TestClient, pm_auth_headers: dict, member_auth_headers: dict):
    ...
    assert client.get(f"/api/tasks/{task['id']}", headers=member_auth_headers).status_code in [403, 404]
    assert client.patch(f"/api/tasks/{task['id']}", json={"title": "hijacked"},
                        headers=member_auth_headers).status_code in [403, 404]
    assert client.post(f"/api/tasks/{task['id']}/cycle-status",
                       headers=member_auth_headers).status_code in [403, 404]
    assert client.delete(f"/api/tasks/{task['id']}", headers=member_auth_headers).status_code in [403, 404]
