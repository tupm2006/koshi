    ...
    <div class="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl p-5 md:p-6 shadow-2xl border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 flex flex-col max-h-[88vh]">
      ...
      <div class="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
        <div class="flex items-center gap-2.5">
          <div class="p-2 rounded-xl bg-sky-50 dark:bg-sky-500/10 border border-sky-200 dark:border-sky-500/30 text-sky-600 dark:text-sky-400">
      ...
      <div class="flex-1 overflow-y-auto py-4 space-y-4 text-xs">
        <div>
          <label for="vue-meeting-notes-input" class="block font-mono text-slate-700 dark:text-zinc-300 mb-1.5 font-semibold">Raw Meeting Notes / Transcript *</label>
          <textarea
            id="vue-meeting-notes-input"
            v-model="rawNotes"
            rows="4"
            placeholder="Paste raw unstructured meeting notes here..."
            class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-zinc-700 rounded-xl p-3 text-xs text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-500 focus:outline-none focus:border-sky-500 font-mono"
          ></textarea>
        </div>

        <button
          type="button"
          class="w-full py-2 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-mono font-medium text-xs flex items-center justify-center gap-1.5 cursor-pointer transition shadow-xs disabled:opacity-50"
          @click="handleExtract"
        ...
        <div v-if="result" class="space-y-3 pt-2 border-t border-slate-200 dark:border-zinc-800 animate-in fade-in">
          <!-- Main Topics -->
          <div v-if="result.main_topics && result.main_topics.length > 0" class="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-zinc-800">
            <h4 class="font-mono text-slate-500 dark:text-zinc-400 font-semibold mb-1.5 uppercase text-[10px] tracking-wider">Main Topics Discussed:</h4>
            <ul class="list-disc list-inside space-y-1 text-slate-800 dark:text-zinc-200">
          ...
          <div v-if="result.action_items && result.action_items.length > 0" class="space-y-2">
            <h4 class="font-mono text-slate-500 dark:text-zinc-400 font-semibold uppercase text-[10px] tracking-wider">Extracted Action Items:</h4>
            <div class="grid grid-cols-1 gap-2">
              <div
                v-for="(item, idx) in result.action_items"
                :key="idx"
                class="p-2.5 rounded-lg bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 flex items-center justify-between gap-2"
              >
                <div class="flex items-center gap-2 min-w-0">
                  <span class="w-1.5 h-1.5 rounded-full bg-sky-500 shrink-0"></span>
                  <span class="font-medium text-slate-800 dark:text-zinc-200 truncate">{{ item.title }}</span>
                </div>
                <div class="flex items-center gap-2 shrink-0 font-mono text-[10px]">
                  <span class="text-slate-600 dark:text-zinc-400 flex items-center gap-1">
                    <User class="w-3 h-3 text-slate-400 dark:text-zinc-500" />
                    {{ item.assignee_name || 'Unassigned' }}
                  </span>
                  <span class="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 border border-slate-300 dark:border-zinc-700">
          ...
          <!-- Key Decisions -->
          <div v-if="result.key_decisions && result.key_decisions.length > 0" class="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/40 text-emerald-800 dark:text-emerald-200">
