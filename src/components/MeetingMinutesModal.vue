  ...
  <div class="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-3 md:p-6 animate-in fade-in duration-100">
    <div class="glass-panel w-full max-w-2xl rounded-2xl p-5 md:p-6 shadow-2xl border border-zinc-700/60 text-zinc-100 flex flex-col max-h-[88vh]">
      ...
      <div class="flex-1 overflow-y-auto py-4 space-y-4 text-xs">
        <div>
          <label for="vue-meeting-notes-input" class="block font-mono text-zinc-300 mb-1.5 font-semibold">Raw Meeting Notes / Transcript *</label>
          <textarea
        ...
        <div v-if="result" class="space-y-3 pt-2 border-t border-zinc-800 animate-in fade-in">
          <!-- Main Topics -->
          <div v-if="result.main_topics && result.main_topics.length > 0" class="p-3 rounded-xl bg-zinc-900/60 border border-zinc-800">
          ...
          <div v-if="result.action_items && result.action_items.length > 0" class="space-y-2">
            ...
            <div class="grid grid-cols-1 gap-2">
              <div
                ...
                :key="idx"
                class="p-2.5 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-between gap-2"
              >
          ...
          <!-- Key Decisions -->
          <div v-if="result.key_decisions && result.key_decisions.length > 0" class="p-3 rounded-xl bg-emerald-950/20 border border-emerald-900/40 text-emerald-200">
